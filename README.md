...
| `three` / `four` | 3 / 4 fingers | *unbound* |
...
| `l_shape` | thumb + index at a right angle | *unbound* |
| `rock` | index + pinky | *unbound* |
...
267 tests, under a second, **no camera, no microphone and no model download required**. Hand poses are synthesized as landmark geometry ([tests/conftest.py](tests/conftest.py)), so the whole vision path is exercised on CI hardware that has never seen a webcam.
