...
class VisionPipeline:
    ...
    def __init__(self, cfg: Config, *, classifier: Classifier | None = None, clock=time.monotonic) -> None:
        ...
        self.swipes: SwipeDetector | None = None
        self._holds: dict[str, int] = {}
        self._hold_emitted_at: dict[str, float] = {}
    ...
    def apply_config(self, cfg: Config) -> None:
        ...
        swipe = cfg.settings.swipe
        self.swipes = SwipeDetector(
            min_distance=swipe.min_distance,
            min_velocity=swipe.min_velocity,
            axis_ratio=swipe.axis_ratio,
            refractory_ms=swipe.refractory_ms,
            window_ms=swipe.window_ms,
        ) if swipe.enabled else None

        # Which gestures must be *held* before they count, and for how long.
        # Read from the bindings the same way arming reads its gesture: the
        # pipeline needs the durations to time them, but it still emits events
        # rather than deciding anything.
        self._holds = {
            b.trigger.value: b.hold_ms
            for b in cfg.bindings
            if b.enabled and b.hold_ms and b.trigger.type == "gesture"
        }
    ...
    def process(self, hand: HandFrame, *, captured_at: float | None = None, aspect: float = 0.75) -> FrameResult:
        ...
        # Motion first: a swipe is decided before the pose, because while the
        # hand is travelling there is no pose worth reporting.
        swipe = None
        speed = 0.0
        if self.swipes is not None:
            swipe = self.swipes.push(hand.landmarks if hand.found else None, now=now)
            speed = self.swipes.speed
            if swipe is not None:
                events.append(
                    Event(
                        kind=EventKind.MOTION,
                        value=swipe.direction,
                        captured_at=swipe.started_at,
                        emitted_at=self._clock(),
                        payload={"distance": round(swipe.distance, 3),
                                 "velocity": round(swipe.velocity, 2)},
                    )
                )

        moving = bool(self.swipes is not None and speed >= self.cfg.settings.swipe.motion_gate)
        # The motion gate: a hand crossing the frame is not holding a pose.
        # Feeding `none` while it travels both suppresses the accidental pose
        # and re-arms the gesture for when the hand settles again.
        label = NONE_LABEL if (moving or swipe is not None) else classification.label
        transition = self.stabilizer.push(label, captured_at=captured_at)
        ...
        hold_progress = self._holds_progress(events, captured_at, now)
    ...
    def _holds_progress(self, events: list[Event], captured_at: float, now: float) -> float:
        """Emit a second event once a held-binding pose has been held long enough.

        The first event (the ordinary transition) is what the dispatcher refuses
        with "holding"; this one carries ``hold: True`` and is the one allowed
        through. Splitting it in two keeps the decision in the dispatcher and
        the timing in the pipeline, which is where the clock already is.
        """
        label = self.stabilizer.stable_label
        required = self._holds.get(label)
        if not required:
            return 0.0
        held = self.stabilizer.held_ms(now)
        progress = min(1.0, held / max(required, 1))
        if progress >= 1.0 and self.stabilizer.stable_since > self._hold_emitted_at.get(label, 0.0):
            self._hold_emitted_at[label] = self.stabilizer.stable_since
            events.append(
                Event(
                    kind=EventKind.GESTURE,
                    value=label,
                    captured_at=captured_at,
                    emitted_at=self._clock(),
                    payload={"hold": True, "held_ms": round(held)},
                )
            )
        return progress
