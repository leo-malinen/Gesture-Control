...
class RuleClassifier:
    ...
    def _label(self, *, thumb, index, middle, ring, pinky, pinching, pts, spread, hand_span) -> str:
        ...
        if thumb and index and count == 2:
            # Index extended with the thumb also out. The thumb's position here
            # is mostly incidental - plenty of people point with the thumb
            # relaxed away from the palm - so the index is what carries the
            # intent. (This used to split into `l_shape` at a wide angle; with
            # that pose retired, collapsing to `one` is the least surprising
            # reading, and it fixes the old behaviour of calling it `two`.)
            return "one"
