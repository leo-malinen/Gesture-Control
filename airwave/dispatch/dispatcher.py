...
class Dispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self.observers: list[Callable[[DispatchRecord], None]] = []
        """Read-only taps on every decision, for UIs (the web hub subscribes here).

        Observers see the record *after* it is final and cannot change anything
        about dispatch - they are handed the outcome, not asked for one. Kept as
        a list rather than a single slot so the overlay and the hub can both
        watch without knowing about each other."""
    ...
    def handle(self, event: Event) -> DispatchRecord:
        ...
        if event.kind is EventKind.SPEECH and event.payload.get("dictation"):
            return self._handle_dictation(event, now)
        ...
        if binding.hold_ms and not event.payload.get("hold"):
            # The pose was recognized but not yet *held*. Not an error - this is
            # the normal first half of every held binding, and the log shows it
            # so the user can see the hold is being counted.
            return self._record(event, OUTCOME_HOLDING, binding.name,
                                detail=f"hold {binding.hold_ms / 1000:.1f}s to fire")
    ...
    def _handle_dictation(self, event: Event, now: float) -> DispatchRecord:
        """Type a dictated phrase. Deliberately not a binding.

        Dictation is a mode, not a trigger: while it is on, whatever you say
        becomes text. Routing it through the binding table would mean declaring
        every possible sentence in YAML.
        """
        from ..config import Action  # noqa: PLC0415 - avoids a cycle at import time

        if not self.executor.controls.get_dictation():
            # An utterance that was still being transcribed when dictation was
            # switched off. Typing it now would be a surprise.
            return self._record(event, OUTCOME_PAUSED, "dictation", "dictation was stopped")

        text = event.value
        if self.cfg.settings.speech.dictation_trailing_space:
            text = f"{text} "
        result = self.executor.execute(Action("text"), payload={"text": text})
        return self._finish(event, None, result, now, name="dictation")
    ...
    def _record(self, event: Event, outcome: str, binding: str | None = None, detail: str = "",
        ...
        for observe in self.observers:
            try:
                observe(record)
            except Exception:  # noqa: BLE001 - a broken UI must not break dispatch
