...
class ActionExecutor:
    ...
    def _do_mode(self, action: Action, payload: dict) -> str:
        ...
        getter = {"mouse": self.controls.get_mouse, "arming": self.controls.get_armed,
                  "pause": self.controls.get_paused,
                  "dictation": self.controls.get_dictation}[action.target or "mouse"]
        setter = {"mouse": self.controls.set_mouse, "arming": self.controls.set_armed,
                  "pause": self.controls.set_paused,
                  "dictation": self.controls.set_dictation}[action.target or "mouse"]
        value = {"on": True, "off": False}.get(action.verb, not getter())
        setter(value)
        return f"{action.target} = {value}"

    def _do_text(self, action: Action, payload: dict) -> str:
        """Type a string. Dictation routes its transcripts through here too,
        passing the text in the payload rather than baking it into the action."""
        text = payload.get("text", action.text)
        if not text:
            return "text action with nothing to type"
        self.backend.type_text(str(text))
        return f"typed {len(str(text))} chars"

    def _do_system(self, action: Action, payload: dict) -> str:
        if action.target == "sleep":
            return self.backend.sleep_machine()
        return f"unknown system target {action.target!r}"
