...
SPEECH_MODEL_URL = "https://alphacephei.com/vosk/models/vosk-model-small-en-us-0.15.zip"
SPEECH_MODEL_DIR = "vosk-model-small-en-us-0.15"


def resolve_speech_model(explicit: str | None = None) -> Path | None:
    """Find a local vosk model: argument > env > ./models > cache.

    Looking locally first matters. ``vosk.Model(lang="en-us")`` downloads a
    model on first construction, which turns the first dictation of a session
    into a silent 40MB fetch - and fails outright on a machine whose Python
    cannot verify TLS. An explicit, pre-fetched model is the honest path.
    """
    if explicit:
        return Path(explicit).expanduser()
    env = os.environ.get("AIRWAVE_SPEECH_MODEL")
    if env:
        return Path(env).expanduser()
    for candidate in (
        Path.cwd() / "models" / SPEECH_MODEL_DIR,
        Path(__file__).resolve().parent.parent.parent / "models" / SPEECH_MODEL_DIR,
        Path.home() / ".cache" / "airwave" / SPEECH_MODEL_DIR,
    ):
        if candidate.is_dir():
            return candidate
    return None
...
def fetch_speech_model(dest: str | None = None) -> Path:
    """Download and unpack the small English vosk model (~40MB), once."""
    import zipfile  # noqa: PLC0415

    from ..fetch import download_file  # noqa: PLC0415

    target = Path(dest).expanduser() if dest else Path.cwd() / "models"
    target.mkdir(parents=True, exist_ok=True)
    final = target / SPEECH_MODEL_DIR
    if final.is_dir():
        return final

    archive = target / f"{SPEECH_MODEL_DIR}.zip"
    log.info("downloading speech model to %s", archive)
    download_file(SPEECH_MODEL_URL, archive)
...
class VoskEngine:
    ...
    def __init__(self, model_path: str | None = None, samplerate: int = 16000) -> None:
        ...
        SetLogLevel(-1)
        local = resolve_speech_model(model_path)
        if local is None:
            raise FileNotFoundError(
                "no local vosk model found. Fetch one once with:\n"
                "  python -m airwave fetch-speech-model\n"
                "or set settings.speech.model_path to a model directory"
            )
        self._model = Model(str(local))
...
class SpeechListener:
    ...
    def __init__(
        ...
        *,
        ...
        on_dictation: Callable[[str, float], None] | None = None,
    ) -> None:
        self.engine = engine
        ...
        """Free-transcription mode. While on, the wake word is bypassed and
        every utterance becomes text instead of being matched against the
        binding table - you are talking *to* the document, not to the app."""
    ...
    def _handle(self, utterance: Utterance) -> None:
        ...
        if self.dictation:
            # No wake word: requiring one before every sentence would make
            # dictation unusable, and the mode itself is the consent.
            if self.on_dictation:
                self.on_dictation(text, utterance.started_at)
            return
