...
settings:
  ...
  # A swipe fires when the palm crosses `min_distance` of the frame faster
  # than `min_velocity`, travelling mostly along one axis. Raise either if
  # ordinary hand movement sets it off.
  #
  # motion_gate suppresses pose recognition while the hand moves this fast,
  # so swiping with an open palm does not also fire the open_palm binding.
  swipe:
    enabled: true
  ...
  # Also required for dictation (typing what you say). Install an engine:
  #   pip install vosk
  speech:
    enabled: false
...
bindings:
  ...
  # Swipes are a separate trigger type, because a motion is not a held pose.
  ...
  # Dictation types what you say at the cursor. Needs speech enabled above.
  # While it is on, the wake word is bypassed and everything you say is typed.
  ...
  # `hold_ms` makes a binding require the pose to be held before it fires -
  # worth it for anything you would hate to trigger by accident. `system`
  # actions are a bounded capability, so they need no allow_shell.
