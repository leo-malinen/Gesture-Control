...
from ..vision.swipe import MOTION_VALUES
from .errors import ConfigIssue, suggest

ACTION_TYPES = ("key", "hotkey", "mouse_move", "mouse_click", "scroll", "shell",
                "mode", "text", "system", "noop")
SOUND_VALUES = ("clap", "double_clap")
MODE_TARGETS = ("mouse", "arming", "pause", "dictation")
SYSTEM_TARGETS = ("sleep",)
"""Deliberately tiny. `system` exists so that suspending the machine does not
require switching on `allow_shell` - a bounded capability is a far smaller
thing to grant than arbitrary command execution."""
...
@dataclass(slots=True)
class SwipeSettings:
    """Dynamic-gesture detection (PRD stretch goal, now shipped).

    ``motion_gate`` is the speed above which pose classification is suppressed.
    It is what stops a swipe performed with an open palm from also firing
    whatever open_palm is bound to.
    """
    ...
    window_ms: int = 400
    motion_gate: float = 0.6

    _KEYS = ("enabled", "min_distance", "min_velocity", "axis_ratio",
             "refractory_ms", "window_ms", "motion_gate")

    @classmethod
    def _parse(cls, ctx: _Ctx, node: Any, path: str) -> "SwipeSettings":
        _unknown_keys(ctx, node, path, cls._KEYS)
        out = cls(
            enabled=_get(ctx, node, path, "enabled", True, bool),
            min_distance=_get(ctx, node, path, "min_distance", 0.22, float),
            min_velocity=_get(ctx, node, path, "min_velocity", 0.9, float),
            axis_ratio=_get(ctx, node, path, "axis_ratio", 1.6, float),
            refractory_ms=int(_get(ctx, node, path, "refractory_ms", 700, int)),
            window_ms=int(_get(ctx, node, path, "window_ms", 400, int)),
            motion_gate=_get(ctx, node, path, "motion_gate", 0.6, float),
        )
        _bounded(ctx, out.min_distance, f"{path}.min_distance", _line_of(node, "min_distance"),
                 minimum=0.05, maximum=1.0, name="min_distance")
        _bounded(ctx, out.min_velocity, f"{path}.min_velocity", _line_of(node, "min_velocity"),
                 minimum=0.1, maximum=10.0, name="min_velocity")
        _bounded(ctx, out.axis_ratio, f"{path}.axis_ratio", _line_of(node, "axis_ratio"),
                 minimum=1.0, maximum=6.0, name="axis_ratio")
        _bounded(ctx, out.motion_gate, f"{path}.motion_gate", _line_of(node, "motion_gate"),
                 minimum=0.0, maximum=10.0, name="motion_gate")
        _bounded(ctx, out.refractory_ms, f"{path}.refractory_ms", _line_of(node, "refractory_ms"),
                 minimum=0, maximum=5000, name="refractory_ms")
        return out
...
class SpeechSettings:
    ...
    dictation_trailing_space: bool = True

    _KEYS = ("enabled", "engine", "model_path", "wake_word", "wake_engine",
             "listen_window_s", "similarity", "dictation_trailing_space")
    ...
    def _parse(cls, ctx: _Ctx, node: Any, path: str) -> "SpeechSettings":
        ...
        out = cls(
            ...
            dictation_trailing_space=_get(ctx, node, path, "dictation_trailing_space", True, bool),
        )
...
class Settings:
    ...
    swipe: SwipeSettings = field(default_factory=SwipeSettings)
    ...
    def _parse(cls, ctx: _Ctx, node: Any, known_gestures) -> "Settings":
        ...
        if not isinstance(node, dict):
            ...
            node = {}
        _unknown_keys(ctx, node, path, list(cls._SCALARS) + ["arming", "mouse", "swipe", "audio", "speech"])
        out = cls(
            ...
            swipe=SwipeSettings._parse(ctx, node.get("swipe") or {}, f"{path}.swipe"),
...
class Binding:
    ...
    """Require the pose to be held this long before the action fires.

    A second, per-binding safety gate independent of arming: the stability
    window proves the classifier agrees, this proves the *user* meant it. Worth
    it for anything you would hate to trigger by accident - suspending the
    machine being the obvious case."""
    line: int | None = None
...
def _parse_trigger(ctx: _Ctx, node: Any, path: str, known_gestures, speech_phrases: list[str]) -> Trigger:
    ...
    elif ttype == EventKind.MOTION.value:
        if value not in MOTION_VALUES:
            ctx.add(f"{path}.value", f"unknown motion {value!r}", line,
                    suggest(value, MOTION_VALUES, "known motions"))
...
def _parse_action(ctx: _Ctx, node: Any, path: str, allow_shell: bool) -> Action:
    ...
    if atype == "mode":
        ...
        # YAML 1.1 reads bare `on` and `off` as booleans, so `verb: on` arrives
        # here as True. Refusing it would be technically correct and wildly
        # unhelpful - `on`/`off` are the natural words for this field, and the
        # user cannot see the type coercion that mangled them.
        raw_verb = node.get("verb")
        if isinstance(raw_verb, bool):
            node = dict(node) | {"verb": "on" if raw_verb else "off"}
    ...
    if atype == "text":
        ...
        if not isinstance(raw, str) or not raw:
            ctx.add(f"{path}.text", "'text' action requires a non-empty 'text' string",
                    _line_of(node, "text"), 'example: text: "kind regards,"')
        else:
            text = raw

    if atype == "system":
        target = _get(ctx, node, path, "target", None, str, choices=SYSTEM_TARGETS)
        if target is None:
            ctx.add(f"{path}.target", "'system' action requires 'target'", _line_of(node),
                    f"one of: {', '.join(SYSTEM_TARGETS)}")
    ...
    *,
    ...
    for i, raw in enumerate(raw_bindings):
        ...
        if not isinstance(raw, dict):
            ...
            continue
        _unknown_keys(ctx, raw, path,
                      ("name", "trigger", "action", "enabled", "cooldown_ms", "requires_arm", "hold_ms"))
        ...
        if hold is not None and (isinstance(hold, bool) or not isinstance(hold, (int, float))):
            ctx.add(f"{path}.hold_ms", f"expected a number, got {hold!r}", _line_of(raw, "hold_ms"))
            hold = None
        if hold is not None:
            _bounded(ctx, hold, f"{path}.hold_ms", _line_of(raw, "hold_ms"),
                     minimum=100, maximum=15000, name="hold_ms")
            if trigger.type != EventKind.GESTURE.value:
                ctx.add(f"{path}.hold_ms", f"hold_ms only applies to gesture triggers, not {trigger.type!r}",
                        _line_of(raw, "hold_ms"),
                        "a sound or motion is instantaneous - there is nothing to hold")
                hold = None
