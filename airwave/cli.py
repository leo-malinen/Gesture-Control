...
def cmd_check(args) -> int:
    ...
    for binding in cfg.bindings:
        ...
        if binding.hold_ms:
            flags.append(f"hold {binding.hold_ms / 1000:g}s")
        if binding.cooldown_ms is not None:
            flags.append(f"cooldown {binding.cooldown_ms}ms")
        if binding.requires_arm is False:
            flags.append("no arming")
...
def cmd_gestures(args) -> int:
    ...
    if cfg and cfg.custom_gestures:
        ...
        for name in cfg.custom_gestures:
            print(f"  {name}")
    from .vision.swipe import MOTION_HELP, MOTION_VALUES  # noqa: PLC0415

    print("\nmotions - bind with trigger type 'motion':\n")
    for name in MOTION_VALUES:
        print(f"  {name:<12} {MOTION_HELP.get(name, '')}")
...
def cmd_doctor(args) -> int:
    ...
    # Speech only matters if the config asks for it, so this is never a [FAIL].
    try:
        from .audio.speech import resolve_speech_model  # noqa: PLC0415
        ...
        if engine_installed and model:
            print(f"[ ok ] speech model at {model}")
        elif engine_installed:
            print("[warn] vosk is installed but no model was found\n"
                  "       fix: python -m airwave fetch-speech-model")
        else:
            print("[warn] no speech engine (dictation and voice commands unavailable)\n"
                  "       fix: pip install vosk && python -m airwave fetch-speech-model")
    except Exception:  # noqa: BLE001
        pass
...
def cmd_fetch_speech_model(args) -> int:
    """One-time download of the vosk English model, needed for dictation."""
    from .audio.speech import SPEECH_MODEL_URL, fetch_speech_model, resolve_speech_model  # noqa: PLC0415

    existing = resolve_speech_model()
    if existing and not args.force:
        print(f"already present: {existing}")
        return 0
    print(f"downloading {SPEECH_MODEL_URL}\n         -> models/  (~40 MB)")
    try:
        path = fetch_speech_model(args.out)
    except Exception as exc:  # noqa: BLE001
        print(f"download failed: {exc}", file=sys.stderr)
        return 1
    print(f"done: {path}\n\nEnable dictation with settings.speech.enabled: true")
    return 0
...
def cmd_hub(args) -> int:
    """Run the app with the web dashboard instead of the OpenCV window."""
    from .app import AirwaveApp  # noqa: PLC0415
    ...
    if args.open:
        # Deferred so the server is listening before the browser asks.
        import threading  # noqa: PLC0415
        import webbrowser  # noqa: PLC0415

        def launch():
            time.sleep(1.0)
            if app.hub_server is not None:
                webbrowser.open(app.hub_server.url)
    ...
    dispatch_stats = app.dispatcher.stats
    print(
        f"\n{stats.frames} frames in {stats.elapsed_s:.1f}s  ({stats.fps:.1f} fps)\n"
        f"fired {dispatch_stats.fired}  cooldown-suppressed {dispatch_stats.cooldown}  "
        f"unarmed {dispatch_stats.unarmed}  unbound {dispatch_stats.unbound}"
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    hub = sub.add_parser("hub", help="run with the web dashboard (live feed + event log)")
    hub.add_argument("--port", type=int, default=8760, help="localhost port (default 8760)")
    hub.add_argument("--open", action="store_true", help="open a browser once the server is up")
    hub.add_argument("--window", action="store_true", help="also show the OpenCV preview window")
    hub.add_argument("--dry-run", action="store_true", help="log actions instead of performing them")
    hub.add_argument("--no-audio", action="store_true", help="skip the microphone entirely")
    hub.add_argument("--arm", action="store_true", help="force arming mode on")
    hub.add_argument("--mouse", action="store_true", help="start with mouse control on")
    hub.add_argument("--frames", type=int, help="stop after N frames (for testing)")
    ...
    fetch_speech = sub.add_parser("fetch-speech-model",
                                  help="download the vosk speech model (one time, for dictation)")
    fetch_speech.add_argument("--out", help="destination directory")
    fetch_speech.add_argument("--force", action="store_true")
    fetch_speech.set_defaults(func=cmd_fetch_speech_model)
    ...
    record.add_argument("label", help="gesture name, e.g. my_sign")
